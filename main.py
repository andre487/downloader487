...
@click.option('--s3-access-file', default=os.path.expanduser('~/.tokens/s3-access'))
@click.option('--s3-secret-file', default=os.path.expanduser('~/.tokens/s3-secret'))
@click.option('--no-cleanup', is_flag=True)
@click.option('--no-s3', is_flag=True)
def main(
    url: List[str], download_dir: str, playlist: bool, no_cleanup: bool,
    s3_endpoint: str, s3_region: str, s3_bucket: str, s3_access_file: str, s3_secret_file: str,
    no_s3: bool,
) -> None:
